...
gives a list of all files in all the subcategory present in the category. This is to prevent hard coding 
any file name or subcategory. The function get_all_files_cat still goes through further filter the list to 
ensure no repetition of the wikidata id. Also the all_category list got from the addition of all the hidden 
and unhidden category list is filtered to remove category repetition.

All these functions ensures that we get all the details from a category/files with or without subcategory. 
The function get_labels_description_subcat is used to extract information present in the cultural property 
which cannot be got from the mediawiki api (iwlink gives the wikidata id) as well as the wikidata label and 
description and other necessary information that might be of use.
'''
